...
impl ContractInteract {
   ...
   async fn setup_mystery_box(&mut self) {
       let winning_rates_list = MultiValueVec::<MultiValue6::<RewardType, EgldOrEsdtTokenIdentifier<StaticApi>, BigUint<StaticApi>, ManagedBuffer<StaticApi>, u64, u64>>::new();
   ...
   async fn create_mystery_box_fail(&mut self, amount :  BigUint::<StaticApi>, expected_result : ExpectError<'_>) {
     //  let amount = BigUint::<StaticApi>::from(0u128);
...
//TESTE
   ...
   {
       let mut interact = ContractInteract::new().await;
       interact.deploy_fail( ExpectError(4, "Invalid token ID")).await;
   ...
   async fn test_create_mystery_box_without_setup()
   {
       ...
       interact.create_mystery_box_fail(amount, ExpectError(4, "The Mystery Box must be set up first")).await;
   ...
   {
       let mut interact = ContractInteract::new().await;
       interact.setup_mystery_box_fail_percentage( ExpectError(4, "The total percentage must be 100%")).await;
